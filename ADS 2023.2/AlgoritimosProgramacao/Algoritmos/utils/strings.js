...
export function eh_letra_maisucula(char){
    const code = char.charCodeAt(0)
...
export function texto_to_upper(texto){
    let novo_texto = ''
...
export function contem_letra(palavra, letra_procurada){
    for (let letra of palavra){
        if (texto_to_upper(letra) === texto_to_upper(letra_procurada)){
            return true
        }
    }
